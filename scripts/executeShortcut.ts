...
import { getEncodedData, getPrivateKeyFromArgs, getRpcUrlByChainId, getShortcut, getWalletFromArgs } from "../src/helpers";
...
async function main() {
    try {
        ...
        const balances = await Promise.all(quoteTokens.map(async (t) => BigInt(await provider.call({to: t, data: erc20Interface.encodeFunctionData('balanceOf', [wallet])})).toString()));
